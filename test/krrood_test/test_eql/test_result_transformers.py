...
``result_stages`` so a meta-inspector can identify how a query is ordered and quantified without
...
def test_result_stages_includes_an_ordering_stage_when_ordered():
    ...
    assert any(
        ...
        for stage in ordered_query.result_stages
    )
...
def test_result_stages_has_no_ordering_stage_when_unordered():
    ...
    assert not any(
        isinstance(stage, Ordering) for stage in an(entity(value)).result_stages
    )
