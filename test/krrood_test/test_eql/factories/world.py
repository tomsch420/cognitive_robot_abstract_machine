    ...
    for connection in world_conf.connections:
        ...
        if parent and child:
            ...
            world.connections.append(
                connection_cls(parent=parent, child=child, world=world)
            )
        ...
        if isinstance(view, DrawerConf):
            ...
            handle = next(
                (
                    ...
                    if isinstance(b, Handle) and b.name == view.handle.name
                ),
                None,
            )
            container = next(
                (
                    b
                    for b in world.bodies
                    if isinstance(b, Container) and b.name == view.container.name
                ),
                None,
            )
            if handle is None or container is None:
                raise ValueError(
                    f"Handle or Container not found for DrawerConf: {view}"
                )
            ...
            for dconf in view.drawers:
                handle = next(
                    (
                        b
                        for b in world.bodies
                        if isinstance(b, Handle) and b.name == dconf.handle.name
                    ),
                    None,
                )
                cont = next(
                    (
                        b
                        for b in world.bodies
                        if isinstance(b, Container) and b.name == dconf.container.name
                    ),
                    None,
                )
                if handle is None or cont is None:
                    raise ValueError(
                        f"Handle or Container not found for Drawer in CabinetConf: {dconf}"
                    )
                drawers.append(Drawer(handle=handle, container=cont, world=world))
            world.views.append(
                Cabinet(container=cab_container, drawers=drawers, world=world)
            )
        else:
