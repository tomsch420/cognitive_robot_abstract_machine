"""
The single English-preposition inventory and the relation recognizer that derives from
it.

The verbalizer keeps one preposition enum (:class:`Prepositions`): the words it emits,
the prepositions a clause links its constituents with, and the lexicon a relational-
field recognizer consults all read from it.
"""
...
def test_inventory_holds_only_one_word_prepositions():
    """
    The inventory is the simple (one-word) closed class — no multi-word complex
    prepositions.
    """
...
def test_emitted_prepositions_are_present():
    """
    The prepositions the verbalizer emits (genitive, aggregate scope, comitative) are
    members.
    """
...
def test_relation_recognizer_reads_a_newly_covered_preposition():
    """
    The relation recognizer derives from the inventory, so a preposition only now
    covered is read off a participle + preposition field name.
    """
...
def test_relation_recognizer_still_rejects_non_relations():
    """
    Adding prepositions does not loosen the participle guard: a noun ending in a
    preposition, and the genitive ``_of``, are still not relations.
    """
