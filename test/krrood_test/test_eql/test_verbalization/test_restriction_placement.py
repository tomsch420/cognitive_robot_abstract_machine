"""
Unit tests for the condition-form registry — the open-closed seam that maps a folded
WHERE conjunct to its surface form and :class:`SurfacePosition`.

The registry is *total*: a standalone fallback catches anything no specific form fits,
so selection never returns ``None`` and there is no residual special case.
"""
...
def test_attribute_predicate_selects_the_whose_form():
    """
    A single-hop, non-boolean attribute comparator → the whose-predicate form (slot
    WHOSE).
    """
...
def test_range_fold_selects_the_whose_range_form():
    """
    A folded lower/upper bound pair on a subject attribute → the whose-range form (slot
    WHOSE).
    """
    thing = variable(_Thing, [])
...
def test_non_foldable_conjunct_falls_back_to_standalone():
    """
    A conjunct that fits no specific form (here: its right side references the subject)
    falls to the standalone fallback — the registry's totality, no ``None``, no residual
    special case.
    """
    thing = variable(_Thing, [])
    form = ConditionForm.most_applicable(
...
def test_selection_is_total():
    """
    Even an unrelated comparator selects a form (the standalone fallback) — never
    ``None``.
    """
    thing = variable(_Thing, [])
