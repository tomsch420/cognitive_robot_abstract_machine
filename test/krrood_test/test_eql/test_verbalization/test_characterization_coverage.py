...
def test_three_co_owned_selection_attributes_fold_into_one_genitive():
    ...
    assert (
        verbalize_expression(a(set_of(employee.name, employee.department, employee.salary)))
...
def test_range_fold_coexists_with_an_unrelated_conjunct():
    ...
    assert (
        verbalize_expression(
            an(entity(robot).where(robot.battery > 10, robot.battery < 90, robot.operational == True))
        )
...
def test_and_nested_inside_or_keeps_both_connectives():
    ...
    assert verbalize_expression(
        an(entity(robot).where(or_(and_(robot.battery > 50, robot.operational == True), robot.battery < 5)))
    ) == (
