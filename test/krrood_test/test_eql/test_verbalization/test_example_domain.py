...
def test_predicates_build_a_verbalization_fragment(cls_name):
    """
    The custom-predicate examples build their verbalization as a
    :class:`VerbalizationFragment` from the shared vocabulary, given the rendered
    fragment for each field.
    """
    cls = getattr(example_domain, cls_name)
    fields = {
