"""
Tests for conjunction factoring: an AND whose conjuncts are all value comparisons on the
*same bare variable* is said as one main clause — *"an Integer is greater than 1, less
than 10, and not 5"* — the subject and the lead copula stated once and only the
predicate tails coordinated (the conjunctive analogue of the *"… is … or …"*
disjunction).

Scoped to a bare variable; an attribute-chain conjunction keeps its per-clause surface.
"""
...
def test_bare_variable_conjuncts_factor_into_a_relative_clause():
    """
    A complementary bound pair folds to *between* inside the clause; the inequality
    keeps its tail.
    """
    x = variable(int, [])
    assert (
...
def test_inequality_tail_shares_the_lead_copula():
    """
    A ``!=`` tail keeps its *"not"* but shares the affirmative lead *"is"*, so the
    copula is not repeated (*"is greater than 1 and not 5"*).
    """
    x = variable(int, [])
...
def test_attribute_chain_conjunction_is_not_factored():
    """
    An attribute-chain subject keeps per-clause rendering — each conjunct names a
    different subject (*"the battery of a Robot"* vs *"the name of the Robot"*), so
    there is no shared subject to factor.
    """
