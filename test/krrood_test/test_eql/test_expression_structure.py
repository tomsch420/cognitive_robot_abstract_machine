"""
Unit tests for the core structural/semantic expression helpers and the query-algebra accessors
extracted out of the verbalization planners.

These exercise query-algebra facts (chain roots, group keys, aggregated selections) directly on
...
def test_root_variable_ids_collects_distinct_variable_roots():
    """Attributes of the same variable share its root; distinct variables contribute distinct ids;
    expressions whose root is not a variable contribute nothing."""
    body = variable(Body, [])
    other = variable(Body, [])
...
def test_entity_aggregated_selections_falls_back_to_generic_for_plain_selection():
    """An Entity whose selection is not an InstantiatedVariable uses the generic per-selection
    rule (its single selection, kept when it is not a group key)."""
    body = variable(Body, [])
