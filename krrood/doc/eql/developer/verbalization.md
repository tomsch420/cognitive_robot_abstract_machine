...
The local centre (`_chain_topic`) is *which referent a chain foregrounds*. An ordinary navigation foregrounds the entity it describes — its outermost relational referent — so a following attribute reads *"its …"*. A **selected quantity** instead foregrounds the *quantity*: *"the average of the **battery** of the Robot …"* is about the battery, not the Robot, so the Robot is **not** the centre and a following Robot attribute is *not* *"its power"* (which would misread as the battery's power) but the explicit *"the power of the Robot"*. So Q1's *"its"* and Q2's bare-attribute reduction share one centre rule rather than fighting: a selected-quantity clause hands the centre to the quantity, suppressing a misleading *"its"* on its owner's other attributes.
