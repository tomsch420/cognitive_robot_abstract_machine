...
class ModelChangeNode(PlanNode):
    ...
    new_parent is the point to which the body should be attached to. If no parent is provided the world root is used.
    Intended as a convenient use for detect. This is just the representation the actual change lies in the executable in
...
class AttachNode(ModelChangeNode):
    """
    Model change that attaches a body to another body (e.g. an object to the
    gripper after grasping). Kept as a distinct type so it can be located by type
    in the plan.
    """
...
class DetachNode(ModelChangeNode):
    """
    Model change that detaches a body from its current parent and re-attaches it
    to the world root (e.g. after placing an object). Kept as a distinct type so
    it can be located by type in the plan.
    """
