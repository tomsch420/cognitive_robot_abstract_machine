...
class AlignPerpendicular(FeatureFunctionGoal):
    ...
    def __post_init__(self):
        ...
        self.observation_expression = cas.less(cas.abs(0 - expr), self.threshold)
...
class HeightGoal(FeatureFunctionGoal):
    ...
    def __post_init__(self):
        ...
        self.add_inequality_constraint(reference_velocity=self.max_vel,
                                       upper_error=self.upper_limit - expr,
                                       lower_error=self.lower_limit - expr,
                                       weight=self.weight,
                                       task_expression=expr,
                                       name=f'{self.name}_constraint')
        self.observation_expression = cas.logic_and(cas.if_less_eq(expr, self.upper_limit, 1, 0),
...
class DistanceGoal(FeatureFunctionGoal):
    ...
    def __post_init__(self):
        ...
        self.add_inequality_constraint_vector(reference_velocities=[self.max_vel] * 3,
                                              lower_errors=[0, 0, 0],
                                              upper_errors=[0, 0, 0],
...
class AngleGoal(FeatureFunctionGoal):
    ...
    def __post_init__(self):
        ...
        self.add_inequality_constraint(reference_velocity=self.max_vel,
                                       ...
                                       task_expression=expr,
                                       name=f'{self.name}_constraint')
        self.observation_expression = cas.logic_and(cas.if_less_eq(expr, self.upper_angle, 1, 0),
