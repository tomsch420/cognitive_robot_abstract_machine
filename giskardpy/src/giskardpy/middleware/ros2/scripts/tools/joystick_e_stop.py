...
class JoystickEStop:
    ...
    def __init__(
        ...
        """
        self.cancel_client = rospy.get_node().create_client(
        ...
        self.joy_sub = rospy.get_node().create_subscription(
    ...
    def joy_cb(self, joy_msg: Joy):
        ...
        if np.any(filtered_buttons):
            rospy.get_node().get_logger().warning(
                f"joystick buttons {np.argwhere(filtered_buttons).tolist()} pressed"
            )
            rospy.get_node().get_logger().warning(self.cancel_msg)
...
def main(args=None):
    ...
    _, joy_msg = wait_for_message(Joy, rospy.get_node(), "/joy")
    num_buttons = len(joy_msg.buttons)

    rospy.get_node().declare_parameter("button_ids", list(range(num_buttons)))
    button_ids = rospy.get_node().get_parameter("button_ids").value

    e_stop = JoystickEStop(num_buttons, button_ids)
    rospy.get_node().get_logger().info("giskard joystick e stop is running")
